...
同一分组内，邮箱地址（忽略大小写和首尾空格）与密码都相同才视为重复；密码不同或分组不同的记录会保留。导入时会跳过重复项，Vercel 的浏览器存储也会在页面加载时清理组内重复记录。VPS 首次启动新版时会清理库中已有的组内重复记录，保留最早的一条；清理前会在数据库旁生成 `mail.db.pre-dedupe-*.db` 一致性备份。
